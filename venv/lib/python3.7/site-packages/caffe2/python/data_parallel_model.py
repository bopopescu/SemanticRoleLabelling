...
def Parallelize_BMUF(
    ...
    model_helper_obj._broadcast_context = None
    model_helper_obj._shared_model = False
    main_dev_opt = core.DeviceOption(model_helper_obj._device_type, main_device)
    ...
    if warmup_iterations is not None:
        ...
        # A net for broadcasting gpu-0 (main shard) parameters after
        ...
        for param_name in viewkeys(model_helper_obj._device_grouped_blobs):
            param = model_helper_obj._device_grouped_blobs[param_name][main_device]
            with core.DeviceScope(main_dev_opt):
                model_helper_obj._warmup_broadcast.Copy(param, _g(param))

    # (Step-0) Initialize momentum parameters on main device.
    for param_name in viewkeys(model_helper_obj._device_grouped_blobs):
        param = model_helper_obj._device_grouped_blobs[param_name][main_device]
        with core.DeviceScope(main_dev_opt):
...
def ExtractPredictorNet(model, inputs, outputs, device):
    ...
    net.
    '''
    main_device = model._devices[0]
    prefix = "{}_{}/".format(model._device_prefix, main_device)
...
def _Broadcast(devices, model, net, param, use_nccl=False):
    ...
    if use_nccl:
        if _IsGPUBlob(model, param):
            main_device_opt = core.DeviceOption(model._device_type, main_dev)
            with core.DeviceScope(main_device_opt):
    ...
    for dev_idx in devices[1:]:
        ...
        with core.DeviceScope(device_opt):
            net.Copy(
                model._device_grouped_blobs[param][main_dev],
...
def _SyncAllParamsDistributed(
    ...
    for param_name in sorted(unique_param_names):
        main_param = model._device_grouped_blobs[param_name][devices[0]]
        ...
        if rendezvous['engine'] == 'GLOO':
            ...
            with core.DeviceScope(cpu_device_opt):
                broadcast([param_cpu])
            with core.DeviceScope(device_opt):
                net.CopyCPUToGPU(param_cpu, main_param)
...
def _PruneParametersForSharing(model):
    ...
    main_prefix = "{}_{}/".format(model._device_prefix, model._devices[0])

    # Remove non-main parameters so that they will not receive parameter
    # update operators.
    model.params = model.GetParams(main_prefix)
...
def _RemapParameterBlobsForSharedModel(model, all_params):
    assert model._shared_model
    main_prefix = "{}_{}/".format(
        model._device_prefix, model._devices[0])
    log.info("Remapping param blobs to main -> {}".format(main_prefix))
    main_params = set(model.GetParams())

    # Remove all but main params
    def modify_ops(net):
        ops = []
        for op in net.Proto().op:
            delete_op = False
            # Delete ops that output non-main version of parameter
            for outp in op.output:
                if outp in all_params and outp not in main_params:
                    delete_op = True
                    log.debug("Delete b/c {}:  {}".format(outp, str(op)))
                    break
            if delete_op:
                continue
            # Remap inputs to point to the main param
            for j, inp in enumerate(op.input):
                if inp in all_params and inp not in main_params:
                    op.input[j] = main_prefix + stripBlobName(inp)
            ops.append(op)
        del net.Proto().op[:]
...
def _AllReduceBlobsDistributed(
    ...
    main_device_opt = core.DeviceOption(model._device_type, devices[0])

    reducing_device_opt = main_device_opt
    ...
    for blob_name in blob_names:
        ...
        if rendezvous['engine'] == 'GLOO':
            ...
            # Step 1: sum blobs from local GPUs to main GPU
            with core.DeviceScope(main_device_opt):
                model.ConstantFill(main_blob, reduced_blob, value=0.0)
                ...
                net.Copy(main_blob, reduced_blob)

            # Step 2: allreduce between all hosts, between main GPUs
            allreduce([reduced_blob])

            with core.DeviceScope(main_device_opt):
                net.Copy(reduced_blob, main_blob)
        ...
        if _IsGPUBlob(model, blob_name):
            with core.DeviceScope(main_device_opt):
                if not isinstance(blobs_group[0], core.GradientSlice):
                    ...
                    main_ns = "{}_{}".format(model._device_prefix, devices[0])
                    '''
                    ...
                    if not skip_idx_concat:
                        grad_idx_concat, _ = net.Concat(
                            ...
                            ["{}/{}_index_concat".format(main_ns, blob_name),
                             "{}/{}_index_splitinfo".format(main_ns, blob_name)],
                            axis=0,
                    ...
                    grad_val_concat, _ = net.Concat(
                        ...
                        ["{}/{}_val_concat".format(main_ns, blob_name),
                         "{}/{}_val_splitinfo".format(main_ns, blob_name)],
...
def _BroadcastComputedParamsSingleHost(devices, model, use_nccl=False):
    ...
    for param_name in model._computed_param_names:
        # Copy from main to others -- averaging would be perhaps better,
...
def _RunComparison(model, blob_name, device=None):
    ...
    with core.DeviceScope(device):
        ...
        comm_world = comparison_net.CreateCommonWorld(
            rendezvous['kv_handler'] or [],
            "initial_sync",
            name=model.net.Proto().name + ".cw_main_select",
            size=rendezvous['num_shards'],
            rank=rendezvous['shard_id'],
            engine=rendezvous['engine'],
            **kwargs
...
def _GPUInterDeviceBatchNormalization(model):
    ...
    bias_grad_blobs = []
    main_device = "cpu_0"
    main_device_option = core.DeviceOption(caffe2_pb2.CPU)

    def _gpuReduce(param, num_devices, main_device, result_blobs=None):
        """
        Reduces results from multiple gpus and distributes the results back
        to each device. This is done by copying values to the main device
        and summing them. The main device result is then copied back to
        each of the devices.

        param: the name of the data (blobs) to reduce
        num_devices: the number of devices
        main_device: the device to copy/compute values on
        ...
        for i in range(num_devices):
            ...
            destination_blobs.append(
                "{}/{}_gpu_{}_copy".format(main_device, param, i))
            added_ops.append(
                core.CreateOperator(
                    "CopyGPUToCPU",
                    source_blobs[i],
        ...
        added_ops.append(
            core.CreateOperator(
                "Sum",
                destination_blobs,
                "{}/{}_combined".format(main_device, param),
                device_option=main_device_option))
        for i in range(num_devices):
            device_option = core.DeviceOption(model._device_type, i)
            added_ops.append(
                core.CreateOperator(
